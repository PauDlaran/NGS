...
class IHM_NGS(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        #self.image_sub = rospy.Subscriber('Cam1', Image, self.callback_image)
        ...
        self.logo_NGS.grid(row=3, column=0, pady=10, padx=20, ipadx = 350,ipady = 610)
    ...
    def readQrCode2(self, event=0):
        data = self.readQrCode()
        self.qr_code2_label.configure(text=str(data))

    def readQrCode3(self, event=0):
        data = self.readQrCode()
        self.qr_code3_label.configure(text=str(data))
    ...
    def ouvre_boite_1(self):
        if self.bouton_ouverture_1.cget("text") == "Ouvrir" and self.bouton_ouverture_1.cget("text_color") != "black" :
            ...
            self.publier_commande("([2.97,2.5,0.09,1.35],[2.09,1,0.5,1])")
        else :
