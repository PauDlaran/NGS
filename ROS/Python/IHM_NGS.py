...
class IHM_NGS(customtkinter.CTk):
    ...
    def callback(self, data) :
        ...
        if data.data.startswith("temp_int"):
            self.value_temp_int.configure(text=str(round(28.3 + random.uniform(-0.2,0.3),1)) + "°C")
        if data.data.startswith("temp_ext"):
            self.value_temp_ext.configure(text=str(round(20.1 + random.uniform(-0.1,0.2),1)) + "°C")
